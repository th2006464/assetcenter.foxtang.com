...
- **匹配顺序**：先按**计算机名**（不区分大小写）；对不上再按**硬件序列号**兜底（接口 `serial_number` ↔ 标准表的序列号列）。仍对不上判为**从未上报**，同样要处理。
  - 标准版向日葵导出没有独立的序列号列，序列号通常被填在**「备注」**里，所以「备注」也作为序列号候选参与匹配；只有值真的能在接口里找到才算匹配上，不会误命中自由文本。
